    ...
    if(isRemove) {
      ...
      if(pickedImage != null) {
        ...
        if(value > (configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)) {
          showCustomSnackBar('${getTranslated('maximum_image_size', Get.context!)} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB', Get.context!);
        } else {
      ...
      for (PlatformFile file in _pickedImageFiles) {
        if (isVideoExtension(file.path ?? '')) {
          ...
          if (thumbnailPath != null && size <= (configModel?.systemGeneralFileUploadMaxSize ?? AppConstants.maxSizeOfASingleFile)) {
            ...
            showCustomSnackBar('${getTranslated('maximum_file_size', Get.context!)} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB', Get.context!);
          }
        } else {
